  ...
  return (
    <SafeAreaView style={s.container}>
      <View style={s.nav}>
        ...
        <Text style={s.navTitle} accessibilityRole="header">HISTORIAL</Text>
        <View style={{ width: 40 }} />
      </View>

      {loading ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}
          accessible accessibilityLabel="Cargando tu historial"
        ...
        <ScrollView contentContainerStyle={{ padding: Spacing.lg }}>
          {/* Récords */}
          <Text style={s.sectionLbl} accessibilityRole="header">🏆 TUS RÉCORDS</Text>
          {records.length === 0 ? (
            <View style={s.empty}>
            ...
            records.map((r) => (
              <View key={r.exercise_name} style={s.recordCard} accessible
                accessibilityLabel={
                  `${r.exercise_name}. Una repetición máxima estimada: ${r.best1RM} kilos. ` +
                  `Peso máximo levantado: ${r.maxWeight} kilos. Máximo de repeticiones: ${r.maxReps}.`
                }>
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map((ss) => (
              <View key={ss.id} style={s.sessionRow} accessible
                accessibilityLabel={
                  `${ss.completed_at ? new Date(ss.completed_at).toLocaleDateString('es-CO', { day: 'numeric', month: 'long' }) : 'Sin fecha'}: ` +
                  `día ${(ss.day_index ?? 0) + 1} del plan, ${ss.exercises_completed ?? 0} ejercicios en ${ss.duration_min ?? 0} minutos`
                }>
...
const s = StyleSheet.create({
  ...
  sectionLbl: { fontFamily: Fonts.bodySemi, fontSize: Type.micro, color: Colors.textMuted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 },
  ...
  recordUnit: { fontFamily: Fonts.body, fontSize: Type.micro, color: Colors.textMuted },
  recordCellLbl: { fontFamily: Fonts.body, fontSize: Type.micro, color: Colors.textMuted, marginTop: 2 },
  ...
  sessionDateTxt: { fontFamily: Fonts.bodySemi, fontSize: Type.micro, color: Colors.accent, textAlign: 'center' },
  sessionTitle: { fontFamily: Fonts.bodyMedium, fontSize: 14, color: Colors.textPrimary },
  sessionMeta: { fontFamily: Fonts.body, fontSize: Type.micro, color: Colors.textMuted, marginTop: 2 },
});
